...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const marketPricesList = document.getElementById("market-prices");
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", (e) => {
      ...
      if (!name || !email || !password) {
        if (signupError) signupError.textContent = "Vul alle velden in.";
        return;
      }
      ...
      auth
        ...
        .then(() => {
          signupForm.reset();
          if (loginTab) loginTab.click(); // terug naar login-tab
        })
        .catch((err) => {
          console.error(err);
          if (signupError) {
            signupError.textContent =
              err.message || "Account aanmaken mislukt.";
          }
        });
    });
  }
  ...
  auth.onAuthStateChanged((user) => {
    ...
    if (user) {
      ...
      if (userTag) {
        userTag.textContent =
          user.displayName || user.email || "Ingelogde gebruiker";
      }
      ...
      if (userTag) userTag.textContent = "Demo user";
  ...
  function initNavigation() {
    const clickable = document.querySelectorAll("[data-page]");
  ...
  function renderCart() {
    ...
    cart.forEach((item) => {
      ...
      const linePrice = price * weight;

      li.textContent = `${item.name} – ${weight} kg × €${price.toFixed(
        2
      )}/kg = €${linePrice.toFixed(2)}`;
  ...
  function initAnalyticsChart() {
    ...
    stockChartInstance = new Chart(ctx, {
      type: "bar",
      data: {
        labels: ["Carbon prepreg T700", "Epoxy resin 2K", "Glass fabric 450"],
        datasets: [
          {
            label: "Stock (kg)",
            data: [1500, 3200, 7800],
    ...
    postForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (file) {
          ...
          fileType = file.type && file.type.startsWith("image/")
            ? "image"
            : "file";
        }
  ...
  function updateTotals() {
    const material =
      parseFloat(summaryMaterialEl ? summaryMaterialEl.textContent : "0") || 0;
    const region = shipRegionInput ? shipRegionInput.value : "NL";
    const weight =
      parseFloat(shipWeightInput ? shipWeightInput.value : "0") || 0;
    const shipping = calculateShipping(region, weight);

    if (summaryShippingEl)
      summaryShippingEl.textContent = shipping.toFixed(2);
    if (summaryTotalEl)
      summaryTotalEl.textContent = (material + shipping).toFixed(2);
  }
  ...
  // =========================
  // LIVE MARKET PRICES (sidebar in index.html)
  // =========================
  function initMarketPrices() {
    if (!marketPricesList) return;

    async function loadPrices() {
      marketPricesList.innerHTML = "<li>Laden van marktdata…</li>";
      ...
      try {
        ...
        data.forEach((row) => {
          ...
          li.innerHTML = `
            <span class="material-symbol">${String(
              symbol
            ).toUpperCase()}</span>
            <span class="material-price">~ ${Number(price).toFixed(
              2
            )} USD</span>
          `;
          marketPricesList.appendChild(li);
        });

        if (!marketPricesList.children.length) {
          marketPricesList.innerHTML =
